...
Communication with the backend happens via an automatically generated client, generated by the hey-api plugin. The client methods can then either be called directly from the components or accessed through a store, which handles caching, logging and notifications. 

The client lives in `src/lib/api/generated`, is git-ignored, and is produced by `npm run api:generate` from the committed `frontend/openapi.json` contract. Generate it before running or building the frontend, and regenerate it whenever the contract changes — see [Overview & Setup](./overview#the-generated-api-client).

The generated client and store methods that mutate objects return a `Response` object that contains `data` and `error`. Store getters return the objects directly, or null in case of an error.
