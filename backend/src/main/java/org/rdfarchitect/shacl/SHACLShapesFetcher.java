...
public class SHACLShapesFetcher {
            ...
            results.forEachRemaining(
                    querySolution -> {
                        ...
                        copySHACLShapeToNewModel(
                                shaclModel,
                                propertyShapesModel,
                                querySolution.get("?propertyShape"));
                        var propertyShape =
                                PropertyShape.builder()
                                        .id(querySolution.get("?propertyShape").toString())
                                        .triples(
                                                modelToTtlString(
                                                        propertyShapesModel, propertyUri, false))
    ...
    /**
     * Returns the order of a property shape. In this context we assume that maximum one order is
     * defined.
     *
    ...
    private double getPropertyShapeOrder(Model propertyShapeModel) {
        var order =
                propertyShapeModel
                        .listObjectsOfProperty(ResourceFactory.createProperty(SHACL.order.getURI()))
                        .toList();
        if (order.isEmpty()) {
            return 0;
        }
     ...
     * @return a List of {@link PropertyShape} objects containing the string representation of the
     *     SHACL shapes referencing the association
     */
        ...
        try (var qexec = QueryExecutionFactory.create(query, shaclModel)) {
            ...
            results.forEachRemaining(
                    querySolution -> {
                        ...
                        copySHACLShapeToNewModel(
                                shaclModel, nodeShapeModel, querySolution.get("?nodeShape"));
                        var nodeShape =
                                NodeShape.builder()
     ...
     */
    private String modelToTtlString(
            Model model, String referencedRessource, boolean includePrefixes) {
        if (model.isEmpty()) {
            return "No property shapes found "
                    + (referencedRessource != null ? "for <" + referencedRessource + ">" : "")
                    + "!";
        }
