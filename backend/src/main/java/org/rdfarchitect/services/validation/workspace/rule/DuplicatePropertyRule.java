...
public class DuplicatePropertyRule implements WorkspaceValidationRule {
    ...
    private static final ValidationSeverity DUPLICATE_SEVERITY = ValidationSeverity.WARNING;

    private static final ValidationSeverity DEFINITION_DIFFERS_SEVERITY = ValidationSeverity.ERROR;

    private static final ValidationSeverity MULTIPLICITY_DIFFERS_SEVERITY =
    ...
    public void validate(WorkspaceValidationContext context, IssueCollector issues) {
        ...
        reportDuplicates(
                attributes, "Attribute is defined in multiple schemas.", occurrences, issues);
        reportDuplicates(
                associations, "Association is defined in multiple schemas.", occurrences, issues);
    ...
    private <T extends ICIMResource> void reportDuplicates(
        ...
        candidates.forEach(
                (uri, candidateOccurrences) ->
                        issues.report(
                                this,
                                DUPLICATE_SEVERITY,
                                uri,
                                message,
                                candidateOccurrences.stream()
                                        .map(occurrence -> occurrences.of(occurrence, null))
                                        .toList()));
    }
}
