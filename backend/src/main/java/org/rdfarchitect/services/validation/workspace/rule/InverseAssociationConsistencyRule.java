...
@Component
public class InverseAssociationConsistencyRule implements WorkspaceValidationRule {

    private static final String ID = "inverse-association-consistency";

    private static final ValidationSeverity INVERSE_DIFFERS_SEVERITY = ValidationSeverity.ERROR;
    ...
    public void validate(WorkspaceValidationContext context, IssueCollector issues) {
        ConsistencyCheck.<ICIMAssociation>of(this, context, issues)
                .aspect(
                        INVERSE_DIFFERS_SEVERITY,
                        "Association has different inverse associations in different schemas.",
                        association -> association.getInverseAssociation().getUri().toString())
                .run(context.getIndex().sharedAssociations());
    }
}
