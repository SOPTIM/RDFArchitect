...
public class UniqueProfileHeaderRule implements WorkspaceValidationRule {
    ...
    private static final ValidationSeverity DUPLICATE_KEYWORD_SEVERITY = ValidationSeverity.ERROR;

    private static final ValidationSeverity DUPLICATE_VERSION_IRI_SEVERITY =
                ...
                (value, profiles) -> {
                    if (profiles.size() > 1) {
                        issues.report(
                                ...
                                message + value,
                                profiles.stream()
                                        .map(profile -> occurrences.of(profile, value))
                                        .toList());
                    }
                });
    }
