...
public class TripleChangeAnalyser {
    ...
    public List<TripleClassChange> compareGraphsDisregardingPackages(
            Graph originalGraph, Graph updatedGraph) {
    ...
    /**
     * Compares a package in the original graph with the same package in the updated graph. If the
     * package is not defined in either graph, it is considered external.
     *
     ...
     * @return A TriplePackageChange object representing the differences in the package, or null if
    ...
    /**
     * Compares all classes without a package and groups them in a default package. Since the
     * default package is not defined in the graph, it can have no changes in the package
     * definition.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
     * @return A TriplePackageChange object representing the differences between the classes, or
     *     null if there are no changes.
     */
    private TriplePackageChange compareDefaultPackage(Graph originalGraph, Graph updatedGraph) {
        var packageChange = TriplePackageChange.builder().label("default").build();
    ...
    /**
     * Compares a class in the original graph with the same class in the updated graph. It compares
     * the properties, attributes, associations, and enum entries of the class.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
     * @param classURI The URI of the class to compare.
     * @return A ClassChange object representing the differences in the class, or null if there are
     *     no changes.
     */
    private TripleClassChange compareClass(
        ...
        var attributeChanges =
                new ArrayList<>(compareAttributes(originalGraph, updatedGraph, classURI));
        if (!attributeChanges.isEmpty()) {
            classChange.setAttributes(attributeChanges);
        }
        var associationsChanges =
                new ArrayList<>(compareAssociations(originalGraph, updatedGraph, classURI));
        if (!associationsChanges.isEmpty()) {
            classChange.setAssociations(associationsChanges);
        }
        var enumEntriesChanges =
                new ArrayList<>(compareEnumEntries(originalGraph, updatedGraph, classURI));
        if (!enumEntriesChanges.isEmpty()) {
            classChange.setEnumEntries(enumEntriesChanges);
        }

        if (classChange.getChanges() == null
                && classChange.getAttributes() == null
                && classChange.getAssociations() == null
                && classChange.getEnumEntries() == null) {
            return null;
        }
        return classChange;
    }
    ...
    /**
     * Compares the properties of a resource in the original graph with the properties of the same
     * resource in the updated graph. Triples with the predicate RDFA.uuid are ignored, as they are
     * originally only present in the InMemoryGraph.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
    ...
    private String normalizeCommentValue(Node value) {
        ...
        if (value.isLiteral()) {
            ...
            var datatype =
                    value.getLiteralDatatypeURI() == null ? "" : value.getLiteralDatatypeURI();
    ...
    /**
     * Compares the attributes, associations, and enum entries of a class in the original graph with
     * the same class in the updated graph. The prefix and class name are excluded from the label of
     * the NestedObjectChange objects.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
     * @param classURI The URI of the class to compare.
     * @return A list of NestedObjectChange objects representing the differences in attributes,
     *     associations, or enum entries.
     */
    private List<TripleResourceChange> compareAttributes(
            Graph originalGraph, Graph updatedGraph, String classURI) {
    ...
    /**
     * Compares the associations of a class in the original graph with the associations of the same
     * class in the updated graph. The prefix is excluded from the label of the NestObjectChange
     * objects.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
     ...
     */
    private List<TripleResourceChange> compareAssociations(
            Graph originalGraph, Graph updatedGraph, String classURI) {
    ...
    /**
     * Compares the enum entries of a class in the original graph with the enum entries of the same
     * class in the updatedgraph. The prefix and class name are excluded from the label of the
     * NestedObjectChange objects.
     *
     * @param originalGraph The original version to compare to.
     * @param updatedGraph The Graph which is compared to the original graph.
     * @param classURI The URI of the class to compare.
     ...
     */
    private List<TripleResourceChange> compareEnumEntries(
            Graph originalGraph, Graph updatedGraph, String classURI) {
    ...
    /**
     * Retrieves all packages present in either of the two input graphs. External packages are also
     * retrieved by their usage as a classCategory in a class.
     *
    ...
    private HashSet<String> getPackages(Graph graph1, Graph graph2) {
        ...
        graph1.find(Node.ANY, RDF.type.asNode(), CIMS.classCategory.asNode())
                .forEachRemaining(triple -> packages.add(triple.getSubject().toString()));
        graph2.find(Node.ANY, RDF.type.asNode(), CIMS.classCategory.asNode())
                .forEachRemaining(triple -> packages.add(triple.getSubject().toString()));
        graph1.find(Node.ANY, CIMS.belongsToCategory.asNode(), Node.ANY)
                .forEachRemaining(triple -> packages.add(triple.getObject().toString()));
        graph2.find(Node.ANY, CIMS.belongsToCategory.asNode(), Node.ANY)
                .forEachRemaining(triple -> packages.add(triple.getObject().toString()));
        return packages;
    }
    ...
    private HashSet<String> getAllClasses(Graph graph1, Graph graph2) {
        ...
        graph1.find(Node.ANY, RDF.type.asNode(), RDFS.Class.asNode())
                .forEachRemaining(triple -> classes.add(triple.getSubject().toString()));
        graph2.find(Node.ANY, RDF.type.asNode(), RDFS.Class.asNode())
                .forEachRemaining(triple -> classes.add(triple.getSubject().toString()));
        return classes;
    }
    ...
    private HashSet<String> getClassesInPackage(Graph graph1, Graph graph2, String packageURI) {
        var classes = new HashSet<String>();
        graph1.find(Node.ANY, CIMS.belongsToCategory.asNode(), NodeFactory.createURI(packageURI))
                .forEachRemaining(triple -> classes.add(triple.getSubject().toString()));
        graph2.find(Node.ANY, CIMS.belongsToCategory.asNode(), NodeFactory.createURI(packageURI))
                .forEachRemaining(triple -> classes.add(triple.getSubject().toString()));
        return classes;
    }

    /**
     * Retrieves all classes without a package from either of the two input graphs. Classes without
     * a package are identified by the absence of the belongsToCategory predicate.
     *
     * @param graph1 The first graph to search for classes.
     * @param graph2 The second graph to search for classes.
    ...
    private HashSet<String> getAttributes(Graph graph1, Graph graph2, String classURI) {
        ...
        graph1.find(Node.ANY, RDFS.domain.asNode(), NodeFactory.createURI(classURI))
                .forEachRemaining(
                        triple -> {
                            if (graph1.contains(
                                    triple.getSubject(),
        ...
        graph2.find(Node.ANY, RDFS.domain.asNode(), NodeFactory.createURI(classURI))
                .forEachRemaining(
                        triple -> {
                            if (graph2.contains(
                                    triple.getSubject(),
    ...
    /**
     * Retrieves all associations of a class from either of the two input graphs. Fetches only one
     * direction of the association, as the other direction is compared in the other class.
     *
    ...
    private HashSet<String> getAssociations(Graph graph1, Graph graph2, String classURI) {
        ...
        graph1.find(Node.ANY, CIMS.associationUsed.asNode(), Node.ANY)
                .forEachRemaining(
                        triple -> {
                            if (graph1.contains(
                                    triple.getSubject(),
        ...
        graph2.find(Node.ANY, CIMS.associationUsed.asNode(), Node.ANY)
                .forEachRemaining(
                        triple -> {
                            if (graph2.contains(
                                    triple.getSubject(),
    ...
    private HashSet<String> getEnumEntries(Graph graph1, Graph graph2, String classURI) {
        ...
        graph1.find(Node.ANY, RDF.type.asNode(), NodeFactory.createURI(classURI))
                .forEachRemaining(triple -> enumEntries.add(triple.getSubject().toString()));
        graph2.find(Node.ANY, RDF.type.asNode(), NodeFactory.createURI(classURI))
                .forEachRemaining(triple -> enumEntries.add(triple.getSubject().toString()));
        return enumEntries;
    }
}
