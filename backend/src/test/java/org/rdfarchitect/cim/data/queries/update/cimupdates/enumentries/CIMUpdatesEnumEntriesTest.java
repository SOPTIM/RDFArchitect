...
public class CIMUpdatesEnumEntriesTest extends CIMUpdatesTestBase {
    ...
    class insertEnumEntry {
        ...
        void insertEnumEntry_invalidNewEnumEntry_throwsException() {
            // Arrange
            CIMEnumEntry newEnumEntry =
                    CIMEnumEntry.builder().label(new RDFSLabel("invalid", "en")).build();
        ...
        @Test
        @DisplayName(
                "Adds comment and stereotype to enum entry if it exists without comment and stereotype")
        ...
        @Test
        @DisplayName(
                "Adds enum entry with comment and correct stereotype if invalid stereotype has been provided")
        void insertEnumEntry_emptyGraphInvalidStereotypeGiven_insertsWithCorrectStereotype() {
            // Arrange
            CIMEnumEntry newEnumEntry =
                    enumEntryOptional.toBuilder().stereotype(new CIMSStereotype("invalid")).build();
            addGraphFromFile(ENUMENTRIES_FILE_PATH);
            addTriple(
                    NodeFactory.createURI(newEnumEntry.getUri().toString()),
                    CIMS.stereotype.asNode(),
                    NodeFactory.createURI("invalid"));
