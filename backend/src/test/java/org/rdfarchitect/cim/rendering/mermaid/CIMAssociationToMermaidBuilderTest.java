...
class CIMAssociationToMermaidBuilderTest {
    ...
    private static final RDFSLabel FROM_LABEL =
            new RDFSLabel(FROM_CLASS_NAME + "." + TO_CLASS_NAME);
    ...
    void build_biDirectionalAssociation_returnsBidirectionalMermaidAssociation() {
        ...
        var cimAssociationMermaidBuilder =
                new CIMAssociationToMermaidBuilder(from, to, uriToUUIDMap);
        ...
        assertThat(result)
                .isEqualTo(
                        "`"
                                + uriToUUIDMap.get(FROM_CLASS_URI.toString())
                                + "` \"M:1\" <--> \"M:1\" `"
                                + uriToUUIDMap.get(TO_CLASS_URI.toString())
                                + "`\n");
    }
    ...
    void
            build_associationWithDifferentMultiplicities_returnsMermaidAssociationWithCorrectMultiplicities(
                    String fromMultiplicity) {
        // Arrange
        setUpAssociations(
                AssociationUsed.YES, fromMultiplicity, AssociationUsed.YES, fromMultiplicity);
        var cimAssociationMermaidBuilder =
                new CIMAssociationToMermaidBuilder(from, to, uriToUUIDMap);
