...
class CimVersionConsistencyRuleTest extends WorkspaceValidationTestBase {
    ...
    @Test
    void validate_mixedCimVersionsInOneSchema_reportsOnlyTheSchemaError() {
        var eq = model();
        addClass(eq, CIM16, "Breaker");
        ...
        assertThat(issues(report, ValidationSeverity.ERROR))
                ...
                .containsExactly("Schema uses classes of multiple CIM versions.");
    }

    @Test
    void validate_everySchemaMixesTheSameVersions_reportsNoWorkspaceError() {
        var eq = model();
        addClass(eq, CIM16, "Breaker");
        addClass(eq, CIM100, "Switch");
        var ssh = model();
        addClass(ssh, CIM16, "Breaker");
        addClass(ssh, CIM100, "Switch");
