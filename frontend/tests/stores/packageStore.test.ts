...
const MOCK_PKG_INTERNAL: api.PackageDto = {
    uuid: PKG_UUID_1,
    label: "Internal Pkg",
};
const MOCK_PKG_EXTERNAL: api.PackageDto = {
    uuid: PKG_UUID_2,
    label: "External Pkg",
};
...
describe("packageStore", () => {
    ...
    describe("getPackages", () => {
        ...
        test("treats missing internalPackageList or externalPackageList as empty arrays", async () => {
            vi.mocked(api.listPackages).mockResolvedValue({
                data: {
                    internalPackageList: undefined,
                    externalPackageList: undefined,
                } as never,
                error: undefined,
            });
        ...
        test("different dataset+graph combinations are cached independently", async () => {
            vi.mocked(api.listPackages)
                ...
                .mockResolvedValueOnce({
                    data: { internalPackageList: [], externalPackageList: [] },
                    error: undefined,
                });
    ...
    describe("addPackage", () => {
        test("calls API with correct arguments", async () => {
            ...
            vi.mocked(api.addPackage).mockResolvedValue({
                data: PKG_UUID_1,
                error: undefined,
            });
        ...
        test("prefers server-issued UUID over the one sent in the DTO", async () => {
            ...
            expect(cached?.internal.some(p => p.uuid === "client-uuid")).toBe(
                false,
            );
        });
    ...
    describe("replacePackage", () => {
        test("calls API with correct arguments", async () => {
            ...
            const updatedPkg: api.PackageDto = {
                uuid: PKG_UUID_1,
                label: "Updated",
            };
    ...
    describe("deletePackage", () => {
        test("early-returns with error if pkg.uuid is missing, without calling API", async () => {
            ...
            const result = await store.deletePackage(
                DATASET_A,
                GRAPH_URI_1,
                pkgWithoutUUID,
            );
        ...
        test("calls API with correct arguments", async () => {
            vi.mocked(api.deletePackage).mockResolvedValue({
                data: undefined,
                error: undefined,
            });
        ...
        test("removes the package from the internal list on success", async () => {
            ...
            expect(cached?.internal.some(p => p.uuid === PKG_UUID_1)).toBe(
                false,
            );
