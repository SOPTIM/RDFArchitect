...
describe("ClassStore", () => {
    ...
    describe("getClasses", () => {
        test("returns classes from the API", async () => {
            ...
            const result = await store.getClasses(DATASET, GRAPH);
        ...
        test("force=true bypasses the cache", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.getClasses(DATASET, GRAPH, false, true);
        ...
        test("internal-only and all-classes variants are cached independently", async () => {
            ...
            await store.getClasses(DATASET, GRAPH, false); // internalOnly
            await store.getClasses(DATASET, GRAPH, true); // all
        ...
        test("returns null when datasetName is empty", async () => {
        ...
        test("returns null when graphURI is empty", async () => {
            expect(await store.getClasses(DATASET, "")).toBeNull();
            expect(api.getClassList).not.toHaveBeenCalled();
        });

        test("returns null on API error", async () => {
            vi.mocked(api.getClassList).mockResolvedValue(err());
            expect(await store.getClasses(DATASET, GRAPH)).toBeNull();
        });
    });
    ...
    describe("getClassInfo", () => {
        test("fetches class details and merges them into both variants", async () => {
            ...
            const result = await store.getClassInfo(DATASET, GRAPH, "uuid-1");
        ...
        test("returns cached details without re-fetching", async () => {
            ...
            await store.getClasses(DATASET, GRAPH, false);
            await store.getClassInfo(DATASET, GRAPH, "uuid-1");
            await store.getClassInfo(DATASET, GRAPH, "uuid-1");
        ...
        test("force=true re-fetches even when details are cached", async () => {
            ...
            await store.getClasses(DATASET, GRAPH, false);
            await store.getClassInfo(DATASET, GRAPH, "uuid-1");
            await store.getClassInfo(DATASET, GRAPH, "uuid-1", true);
        ...
        test("coalesces concurrent detail requests to a single API call", async () => {
            ...
            const p1 = store.getClassInfo(DATASET, GRAPH, "uuid-1");
            const p2 = store.getClassInfo(DATASET, GRAPH, "uuid-1");
        ...
        test("returns null when any param is empty", async () => {
            ...
            expect(await store.getClassInfo(DATASET, "", "uuid-1")).toBeNull();
            expect(await store.getClassInfo(DATASET, GRAPH, "")).toBeNull();
    ...
    describe("addClass", () => {
        test("invalidates the graph cache and returns the new UUID", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            const result = await store.addClass(DATASET, GRAPH, {} as never);
        ...
        test("shows success toast on success", async () => {
            vi.mocked(api.addClass).mockResolvedValue(ok("new-uuid"));
            await store.addClass(DATASET, GRAPH, {} as never);
    ...
    describe("replaceClass", () => {
        test("updates the class in the store locally without invalidating the cache", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.replaceClass(DATASET, GRAPH, "uuid-1", updated);
    ...
    describe("replaceAttribute", () => {
        test("updates the attribute in the store locally", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.replaceAttribute(DATASET, GRAPH, "class-1", {
                uuid: "attr-1",
                label: "updated-attr",
            } as AttributeDto);
    ...
    describe("replaceEnumEntry", () => {
        test("updates the enum entry in the store locally", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.replaceEnumEntry(DATASET, GRAPH, "class-1", {
                uuid: "entry-1",
                label: "updated",
            } as EnumEntryDto);
    ...
    describe("saveCopyClass", () => {
        test("returns early with no error when sources list is empty", async () => {
            const result = await store.saveCopyClass(DATASET, GRAPH, {
                sources: [],
            });
            expect(result.error).toBeNull();
        ...
        test("invalidates graph cache on success with one class", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.saveCopyClass(DATASET, GRAPH, {
    ...
    describe("invalidateGraph", () => {
        ...
        test("does not affect other graphs in the cache", async () => {
            ...
            await store.getClasses(DATASET, GRAPH);
            await store.getClasses(DATASET, "http://other-graph");
    ...
    describe("invalidateDataset", () => {
        test("removes all graphs of a dataset from the cache", async () => {
            vi.mocked(api.getClassList).mockResolvedValue(ok([]));

            await store.getClasses(DATASET, GRAPH);
            await store.getClasses(DATASET, "http://other-graph");
            await store.getClasses("datasetB", GRAPH);
            ...
            // DATASET had 2 graphs invalidated → 2 extra calls; datasetB stays cached
            expect(api.getClassList).toHaveBeenCalledTimes(5);
        });

        test("does not affect datasets other than the invalidated one", async () => {
            vi.mocked(api.getClassList).mockResolvedValue(ok([]));

            await store.getClasses("datasetB", GRAPH);
            store.invalidateDataset(DATASET); // invalidate a different dataset
            await store.getClasses("datasetB", GRAPH);

            // datasetB should still be cached
            expect(api.getClassList).toHaveBeenCalledTimes(1);
        });
    });
});
