...
<script>
    ...
    let { showDialog = $bindable(), existingNames = [] } = $props();

    const workspaceInputId = `workspaceName-${uuidv4()}`;
    ...
    const disableSubmit = $derived(
        !trimmedName || nameExists || nameHasInvalidCharacters,
    );
