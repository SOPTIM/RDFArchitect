...
/**
 * The name of the check a finding came from. The backend sends it along, so a
 * new rule needs no counterpart here.
 */
export function ruleLabel(issue) {
    return issue.ruleLabel ?? issue.ruleId ?? NO_RULE_LABEL;
}

/**
 * A stable identity for a finding, used to remember which rows are expanded
 * across regroupings.
 */
export function issueKey(issue) {
    return [
        issue.ruleId ?? issue.ruleLabel ?? "",
        issue.resourceUri ?? "",
        issue.message ?? "",
    ].join("|");
}
...
export function groupIssues(issues, groupBy) {
    ...
    switch (groupBy) {
        ...
        case GroupBy.RULE:
            return collect(sorted, issue => ({
                key: issue.ruleId ?? issue.ruleLabel ?? NO_KEY,
                label: ruleLabel(issue),
            }));
        case GroupBy.CLASS:
